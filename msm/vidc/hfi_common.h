...
struct venus_hfi_vpu_ops {
	int (*interrupt_init)(struct venus_hfi_device *device, u32 sid);
	int (*setup_ucregion_memmap)(struct venus_hfi_device *device, u32 sid);
	int (*clock_config_on_enable)(struct venus_hfi_device *device,
		u32 sid);
	int (*reset_ahb2axi_bridge)(struct venus_hfi_device *device, u32 sid);
	int (*power_off)(struct venus_hfi_device *device);
	int (*prepare_pc)(struct venus_hfi_device *device);
	int (*raise_interrupt)(struct venus_hfi_device *device, u32 sid);
	bool (*watchdog)(u32 intr_status);
	int (*noc_error_info)(struct venus_hfi_device *device);
	int (*core_clear_interrupt)(struct venus_hfi_device *device);
...
int __clock_config_on_enable_iris1(struct venus_hfi_device *device,
		u32 sid);
int __setup_ucregion_memory_map_iris1(struct venus_hfi_device *device,
		u32 sid);
/* IRIS2 specific */
int __interrupt_init_iris2(struct venus_hfi_device *device, u32 sid);
int __setup_ucregion_memory_map_iris2(struct venus_hfi_device *device,
		u32 sid);
